...
import inspect # For getting function names in logs
...
BASE_JSON_DIR = "https://github.com/Ved-Dixit/GoMedCamp/releases/download/v0.1.0-alpha/info_json.zip"
...
def _download_file_to_temp(url, suffix=''):
    """Downloads a file from a URL to a temporary file and returns its path."""
    tf = None
    temp_file_path = None
    try:
        app.logger.info(f"Downloading file from URL: {url}")
        ...
        with requests.get(url, stream=True, timeout=60) as r: # Added timeout
            ...
            for chunk in r.iter_content(chunk_size=8192):
                tf.write(chunk)
        tf.close() 
        app.logger.info(f"Successfully downloaded {url} to temporary file: {temp_file_path}")
        return temp_file_path
    except requests.exceptions.RequestException as e:
        app.logger.error(f"Error downloading file from {url}: {e}", exc_info=True)
        if tf: tf.close() # Ensure closed
        if temp_file_path and os.path.exists(temp_file_path): os.unlink(temp_file_path)
        return None
    except Exception as e:
        app.logger.error(f"Unexpected error in _download_file_to_temp for {url}: {e}", exc_info=True)
        if tf: tf.close()
        if temp_file_path and os.path.exists(temp_file_path): os.unlink(temp_file_path)
        return None
    ...
    try:
        ...
        if conn:
            with conn.cursor() as cur:
                ...
                app.logger.debug("Users table checked/created.")
                # Camps Table
                cur.execute("""
                ...
                app.logger.debug("Camps table checked/created.")
                # Patients Table
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS patients (
                        ...
                        email VARCHAR(150) NOT NULL, phone_number VARCHAR(20), disease_detected TEXT,
                        area_location VARCHAR(255), organizer_notes TEXT,
                ...
                except psycopg2.Error as _: app.logger.debug("camp_id in patients already nullable or alter failed.")
                try: cur.execute("ALTER TABLE patients ALTER COLUMN created_by_organizer_id DROP NOT NULL;")
                except psycopg2.Error as _: app.logger.debug("created_by_organizer_id in patients already nullable or alter failed.")
                ...
                app.logger.debug("Patients table checked/created/altered.")
                # Camp Registrations Table
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS camp_registrations (
                        id SERIAL PRIMARY KEY, camp_id INTEGER REFERENCES camps(id) ON DELETE CASCADE NOT NULL,
                        user_id INTEGER REFERENCES users(id) ON DELETE CASCADE NOT NULL,
                        registration_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        status VARCHAR(50) DEFAULT 'pending', notes TEXT, UNIQUE (camp_id, user_id));""")
                app.logger.debug("Camp registrations table checked/created.")
                ...
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS connection_requests (
                        ...
                        status VARCHAR(50) DEFAULT 'pending', requested_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        responded_at TIMESTAMP WITH TIME ZONE, UNIQUE (camp_id, organizer_id, local_org_id));""")
                app.logger.debug("Connection requests table checked/created.")
                # Chat Messages Table
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS chat_messages (
                        id SERIAL PRIMARY KEY, connection_request_id INTEGER REFERENCES connection_requests(id) ON DELETE CASCADE NOT NULL,
                        sender_id INTEGER REFERENCES users(id) ON DELETE CASCADE NOT NULL, message_text TEXT NOT NULL,
                        sent_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, read_at TIMESTAMP WITH TIME ZONE);""")
                app.logger.debug("Chat messages table checked/created.")
                # Camp Staff, Medicines, Equipment Tables
                ...
                app.logger.debug("Camp staff table checked/created.")
                ...
                app.logger.debug("Camp medicines table checked/created.")
                ...
                app.logger.debug("Camp equipment table checked/created.")
                # Patient Feedback, Chat Messages, Camp Reviews, Camp Follow-ups
                ...
                app.logger.debug("Patient feedback table checked/created.")
                ...
                app.logger.debug("Patient chat messages table checked/created.")
                ...
                app.logger.debug("Camp reviews table checked/created.")
                ...
                app.logger.debug("Camp follow-ups table checked/created.")
                
                conn.commit()
                app.logger.info("All tables checked/created, alterations attempted, and transaction committed successfully.")
                return True 
        else:
            app.logger.error("Could not create/alter tables due to failed database connection (conn is None).")
            return False
    except psycopg2.Error as e:
        ...
        elif conn is None and not get_db_connection(): 
             pass 
...
def load_indicator_data_for_state(state_name_url_case, indicator_id_req):
    ...
    data_source_path = BASE_JSON_DIR
    is_zip_source = False
    temp_zip_file_to_clean = None

    try:
        if BASE_JSON_DIR.startswith('http://') or BASE_JSON_DIR.startswith('https://'):
            app.logger.info(f"BASE_JSON_DIR is a URL. Attempting to download: {BASE_JSON_DIR}")
            temp_zip_file_to_clean = _download_file_to_temp(BASE_JSON_DIR, suffix='.zip')
            if not temp_zip_file_to_clean:
                app.logger.error(f"Failed to download ZIP file from {BASE_JSON_DIR}")
                return None, full_indicator_name_text
            data_source_path = temp_zip_file_to_clean
            is_zip_source = True
            ...
            app.logger.error(f"BASE_JSON_DIR ('{BASE_JSON_DIR}') is not a valid URL, local ZIP, or local directory.")
            return None, full_indicator_name_text

        if is_zip_source:
            app.logger.info(f"Attempting to load indicator data from ZIP archive: {data_source_path}")
            with zipfile.ZipFile(data_source_path, 'r') as zf:
                state_path_prefix_in_zip = state_name_url_case.replace(os.path.sep, '/') + '/'
                candidate_files = [name for name in zf.namelist() if name.startswith(state_path_prefix_in_zip) and name.lower().endswith('.json') and name.count('/') == state_path_prefix_in_zip.count('/')]
                if not candidate_files:
                    app.logger.warning(f"No JSON files found for state '{state_name_url_case}' in ZIP {data_source_path}")
                    return None, full_indicator_name_text
                for filepath_in_zip in candidate_files:
                    district_name_from_file = standardize_name(filepath_in_zip.split('/')[-1].replace('.json', ''))
                    if not district_name_from_file: continue
                    try:
                        data = json.loads(zf.read(filepath_in_zip).decode('utf-8'))
                        ...
                        if indicator_info:
                            ...
                            all_district_data.append({'district_standardized': district_name_from_file, 'value': pd.to_numeric(value, errors='coerce'), 'indicator_name_text': full_indicator_name_text})
                    except Exception as e_file: app.logger.error(f"Error processing file {filepath_in_zip} from ZIP {data_source_path}: {e_file}", exc_info=True)
        else: # Directory logic
            app.logger.info(f"Loading indicator data from directory: {data_source_path}")
            state_dir_full_path = os.path.join(data_source_path, state_name_url_case) # Path to state specific folder
            if not os.path.isdir(state_dir_full_path):
                 app.logger.warning(f"State JSON directory not found: {state_dir_full_path}")
                 return None, f"Indicator ID {indicator_id_req}"
            for filename in os.listdir(state_dir_full_path):
                if filename.endswith('.json'):
                    district_name_from_file = standardize_name(filename.replace('.json', ''))
                    if not district_name_from_file: continue
                    filepath = os.path.join(state_dir_full_path, filename)
                    try:
                        ...
                        if indicator_info:
                            ...
                            all_district_data.append({'district_standardized': district_name_from_file, 'value': pd.to_numeric(value, errors='coerce'), 'indicator_name_text': full_indicator_name_text})
                    except Exception as e_dir_file: app.logger.error(f"Error processing file {filepath} from directory: {e_dir_file}", exc_info=True)
        
        if not all_district_data: return None, full_indicator_name_text
        df_indicators = pd.DataFrame(all_district_data).dropna(subset=['value'])
        ...
        app.logger.error(f"Generic error in load_indicator_data_for_state for '{state_name_url_case}', indicator '{indicator_id_req}': {e}", exc_info=True)
        return None, f"Indicator ID {indicator_id_req}"
    finally:
        if temp_zip_file_to_clean and os.path.exists(temp_zip_file_to_clean):
            try:
                os.unlink(temp_zip_file_to_clean)
                app.logger.info(f"Cleaned up temporary ZIP file: {temp_zip_file_to_clean}")
            except Exception as e_unlink:
                app.logger.error(f"Error unlinking temporary ZIP file {temp_zip_file_to_clean}: {e_unlink}", exc_info=True)
...
def _get_column_name(df_columns, env_var_value, possible_names_list, column_type_name, csv_path_for_logging):
    ...
    elif env_var_value: app.logger.warning(f"Specified {column_type_name} column '{env_var_value}' not found in {csv_path_for_logging}. Auto-detecting...")
...
def load_geographic_data_from_csv(state_name_standardized_filter):
    data_source_path = CSV_POINTS_PATH
    temp_csv_file_to_clean = None
    gdf_districts = None
    csv_district_col_name_found = None

    try:
        if CSV_POINTS_PATH.startswith('http://') or CSV_POINTS_PATH.startswith('https://'):
            app.logger.info(f"CSV_POINTS_PATH is a URL. Attempting to download: {CSV_POINTS_PATH}")
            temp_csv_file_to_clean = _download_file_to_temp(CSV_POINTS_PATH, suffix='.csv')
            if not temp_csv_file_to_clean:
                app.logger.error(f"Failed to download CSV file from {CSV_POINTS_PATH}")
                return None, None
            data_source_path = temp_csv_file_to_clean
        elif not os.path.isfile(CSV_POINTS_PATH):
            app.logger.error(f"Geographic CSV file not found (local path): {os.path.abspath(CSV_POINTS_PATH)}")
            return None, None

        df_all_geo_points = None
        encodings_to_try = ['utf-16', 'utf-8-sig', 'latin1', 'utf-8']
        for enc in encodings_to_try:
            try:
                df_all_geo_points = pd.read_csv(data_source_path, encoding=enc)
                app.logger.info(f"Successfully loaded geographic CSV: {data_source_path} using '{enc}' encoding.")
                break
            except UnicodeDecodeError: app.logger.warning(f"UnicodeDecodeError with '{enc}' for {data_source_path}. Trying next...")
            except Exception as e_read: app.logger.error(f"Error loading geographic CSV {data_source_path} with '{enc}': {e_read}", exc_info=True); return None, None
        
        if df_all_geo_points is None: app.logger.error(f"Failed to load geographic CSV {data_source_path} with tried encodings."); return None, None
        
        current_csv_state_col = _get_column_name(df_all_geo_points.columns, ENV_CSV_STATE_COL, ['State_Name', 'state_name', 'State', 'state', 'NAME_1', 'ADM1_EN', 'ST_NM'], "state name", data_source_path)
        ...
        if df_state_geo_points.empty: app.logger.warning(f"No geographic data for state '{state_name_standardized_filter}' in {data_source_path}."); return None, None

        csv_district_col_name_found = _get_column_name(df_state_geo_points.columns, ENV_CSV_DISTRICT_COL, ['District_Name', 'district_name', 'District', 'district', 'NAME_2', 'ADM2_EN', 'dt_name', 'Dist_Name'], "district name", data_source_path)
        if not csv_district_col_name_found: return None, None
        current_csv_lat_col = _get_column_name(df_state_geo_points.columns, ENV_CSV_LAT_COL, ['Latitude', 'latitude', 'Lat', 'lat', 'Y', 'y_coord'], "latitude", data_source_path)
        if not current_csv_lat_col: return None, csv_district_col_name_found
        current_csv_lon_col = _get_column_name(df_state_geo_points.columns, ENV_CSV_LON_COL, ['Longitude', 'longitude', 'Lon', 'lon', 'X', 'x_coord'], "longitude", data_source_path)
        if not current_csv_lon_col: return None, csv_district_col_name_found
        ...
        if df_state_geo_points.empty: app.logger.warning(f"No valid lat/lon data for state '{state_name_standardized_filter}'."); return None, csv_district_col_name_found
        
        geometry = gpd.points_from_xy(df_state_geo_points[current_csv_lon_col], df_state_geo_points[current_csv_lat_col])
        cols_to_keep = [col for col in [csv_district_col_name_found, current_csv_state_col] if col in df_state_geo_points.columns]
        gdf_districts = gpd.GeoDataFrame(df_state_geo_points[cols_to_keep], geometry=geometry, crs="EPSG:4326")
        gdf_districts['district_standardized_geo'] = gdf_districts[csv_district_col_name_found].astype(str).apply(standardize_name)
        gdf_districts = gdf_districts[gdf_districts['district_standardized_geo'] != ""]
        if gdf_districts.empty: app.logger.warning(f"GeoDataFrame for state '{state_name_standardized_filter}' empty after removing empty standardized district names."); return None, csv_district_col_name_found
        
        return gdf_districts, csv_district_col_name_found

    except Exception as e:
        app.logger.error(f"Generic error in load_geographic_data_from_csv for state '{state_name_standardized_filter}': {e}", exc_info=True)
        return None, csv_district_col_name_found 
    finally:
        if temp_csv_file_to_clean and os.path.exists(temp_csv_file_to_clean):
            try:
                os.unlink(temp_csv_file_to_clean)
                app.logger.info(f"Cleaned up temporary CSV file: {temp_csv_file_to_clean}")
            except Exception as e_unlink:
                app.logger.error(f"Error unlinking temporary CSV file {temp_csv_file_to_clean}: {e_unlink}", exc_info=True)
...
def row_to_dict(row_raw):
    ...
    for key, value in row.items():
        ...
        elif not isinstance(value, (float, int, str, bool)) and value is not None:
...
def signup():
    ...
    username, email, phone_number, password, user_type, address = \
        data.get('username'), data.get('email'), data.get('phone_number'), \
        data.get('password'), data.get('userType'), data.get('address')
    ...
    if user_type not in ['organizer', 'requester', 'local_organisation']:
        return jsonify({"error": "Invalid user type."}), 400
    if user_type == 'local_organisation' and not address:
        return jsonify({"error": "Address is required for Local Organisation."}), 400
    ...
    try:
        conn = get_db_connection()
        if not conn: 
            app.logger.error(f"[{func_name}] Database connection failed.")
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if cur.fetchone(): return jsonify({"error": "User with this username, email, or phone already exists."}), 409
            
            cur.execute("""INSERT INTO users (username, email, phone_number, password_hash, user_type, address) 
                           VALUES (%s, %s, %s, %s, %s, %s) RETURNING id, username, email, user_type, address, created_at;""",
                        (username, email, phone_number, hashed_password, user_type, address if user_type == 'local_organisation' else None))
            new_user_raw = cur.fetchone()
            if not new_user_raw:
                app.logger.error(f"[{func_name}] User creation failed, no data returned from DB.")
                if conn: conn.rollback()
                return jsonify({"error": "User creation failed unexpectedly."}), 500

            new_user_id = new_user_raw['id']
            if user_type == 'requester':
                cur.execute("""INSERT INTO patients (user_id, name, email, phone_number) VALUES (%s, %s, %s, %s)""",
                            (new_user_id, username, email, phone_number))
                app.logger.info(f"[{func_name}] Basic patient record created for User ID: {new_user_id}")
            
            conn.commit()
            user_data = row_to_dict(new_user_raw)
            user_data['userType'] = user_data.pop('user_type')
            return jsonify({"message": "User created successfully!", "user": user_data}), 201

    except psycopg2.Error as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error: {e}", exc_info=True)
        if hasattr(e, 'pgcode') and e.pgcode == '23505': 
             return jsonify({"error": "A user with this username, email, or phone number already exists."}), 409
        return jsonify({"error": "An error occurred during registration."}), 500
    except Exception as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] An unexpected error occurred: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
            except Exception as e_close: app.logger.error(f"[{func_name}] Error closing connection: {e_close}", exc_info=True)
...
def get_heatmap_data():
    ...
    try:
        ...
        indicator_data_summary = {"count": len(df_indicators) if df_indicators is not None else 0, "available": df_indicators is not None and not df_indicators.empty}
        ...
        geographic_data_summary = {"count": len(gdf_state_districts) if gdf_state_districts is not None else 0, "available": gdf_state_districts is not None and not gdf_state_districts.empty, "message": ""}
        ...
        matched_count = int(merged_gdf['value'].notna().sum())
        ...
        for _, row in merged_gdf.iterrows():
            properties = {
                'original_csv_district_name': row.get(csv_district_col_name, "N/A") if pd.notna(row.get(csv_district_col_name)) else "N/A",
                'district_standardized_geo': row['district_standardized_geo'],
                'value': float(row['value']) if pd.notna(row['value']) else None,
                'indicator_id': indicator_id_req,
                'indicator_name': full_indicator_name
            }
            if row['geometry'] and not row['geometry'].is_empty:
                features_list.append({"type": "Feature", "properties": properties, "geometry": gpd.GeoSeries([row['geometry']]).__geo_interface__['features'][0]['geometry']})
        ...
        response_geojson = {
            ...
            "metadata": {
                "query_state": state_name_req, "query_indicator_id": indicator_id_req, 
                "full_indicator_name": full_indicator_name, "message": final_message,
                "indicator_data_summary": indicator_data_summary, 
                "geographic_data_summary": geographic_data_summary,
                "merge_summary": {
                    "geo_districts_count": geographic_data_summary["count"], 
                    "indicator_districts_count": indicator_data_summary["count"], 
                    "matched_districts_count": matched_count,
                    "unmatched_geo_districts_sample": unmatched_geo_districts[:5], 
                    "unmatched_indicator_districts_sample": unmatched_indicator_districts[:5]
                }
            }
        }
        return jsonify(response_geojson), 200
    except Exception as e:
        app.logger.error(f"[{func_name}] Error processing heatmap data for state {state_name_req}, indicator {indicator_id_req}: {e}", exc_info=True)
...
def create_camp_endpoint():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not new_camp_raw:
                app.logger.error(f"[{func_name}] Camp creation failed, no data returned from DB.")
                if conn: conn.rollback()
...
def get_organizer_camps_endpoint():
    ...
    try:
        ...
        app.logger.error(f"[{func_name}] Database error fetching camps for organizer {organizer_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch camps"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error fetching camps for organizer {organizer_user_id}: {e}", exc_info=True)
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if camp_raw['organizer_id'] != requesting_user_id:
                app.logger.warning(f"[{func_name}] Forbidden access attempt: User {requesting_user_id} tried to access camp {camp_id} owned by {camp_raw['organizer_id']}.")
        ...
        app.logger.error(f"[{func_name}] Database error fetching camp details for camp ID {camp_id}, user {requesting_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch camp details."}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error fetching camp details for camp ID {camp_id}, user {requesting_user_id}: {e}", exc_info=True)
...
def delete_camp_endpoint(camp_id):
    ...
    try:
        ...
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error deleting camp {camp_id} by user {requesting_organizer_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to delete camp."}), 500
    except Exception as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error deleting camp {camp_id} by user {requesting_organizer_id}: {e}", exc_info=True)
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if camp_info['organizer_id'] != requesting_user_id:
                app.logger.warning(f"[{func_name}] Forbidden access attempt: User {requesting_user_id} tried to access resources for camp {camp_id} owned by {camp_info['organizer_id']}.")
            ...
            return jsonify({
                "targetPatients": target_patients, "staffList": staff_list,
                "medicineList": medicine_list, "equipmentList": equipment_list
            }), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error fetching resources for camp {camp_id}, user {requesting_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch camp resources"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error fetching resources for camp {camp_id}, user {requesting_user_id}: {e}", exc_info=True)
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if camp_owner['organizer_id'] != requesting_organizer_id:
                app.logger.warning(f"[{func_name}] Unauthorized attempt to save resources for camp {camp_id} by user {requesting_organizer_id}. Camp owner: {camp_owner['organizer_id']}")
                return jsonify({"error": "Forbidden: You are not the organizer of this camp."}), 403
        ...
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error saving resources for camp {camp_id} by user {requesting_organizer_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to save camp resources"}), 500
    except Exception as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error saving resources for camp {camp_id} by user {requesting_organizer_id}: {e}", exc_info=True)
...
def add_patient_to_camp(camp_id):
    ...
    if not request.is_json: return jsonify({"error": "Missing JSON in request"}), 400
    data = request.get_json()
    patient_name, patient_email = data.get('name'), data.get('email')
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            """
            cur.execute(sql_insert_patient, (
                camp_id, patient_user_id_to_link, patient_name, patient_email,
                data.get('phone_number'), data.get('disease_detected'),
                data.get('area_location'), data.get('organizer_notes'), current_organizer_id
            ))
            new_patient_raw = cur.fetchone()
            if not new_patient_raw:
                app.logger.error(f"[{func_name}] Patient add failed, no data returned from DB.")
                if conn: conn.rollback()
                return jsonify({"error": "Failed to add patient, no data returned."}), 500

            conn.commit()
            patient_dict = row_to_dict(new_patient_raw)
            cur.execute("SELECT name FROM camps WHERE id = %s", (patient_dict['camp_id'],)) # Fetch camp name
        ...
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_camp_patients(camp_id):
    ...
    try:
        ...
        app.logger.error(f"[{func_name}] Database error fetching patients for camp {camp_id}: {e}", exc_info=True)
        return jsonify({"error": "An error occurred while fetching patients."}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error fetching patients for camp {camp_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_my_patient_details():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            return jsonify([row_to_dict(p_raw) for p_raw in patient_profiles_raw]), 200
            
    except psycopg2.Error as e:
        if conn: conn.rollback() # Rollback if commit for UPDATE failed
        app.logger.error(f"[{func_name}] Database error: {e}", exc_info=True)
...
def send_connection_request():
    ...
    if not request.is_json: return jsonify({"error": "Missing JSON in request"}), 400
    data = request.get_json()
    camp_id_str, local_org_id_str = data.get('campId'), data.get('localOrgId')
    ...
    try:
        ...
        return jsonify({"error": "Invalid ID format. All IDs must be integers."}), 400
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not cur.fetchone(): return jsonify({"error": "Local organisation not found or invalid type."}), 404
            ...
            if not new_request_raw:
                app.logger.error(f"[{func_name}] Connection request failed, no data returned from DB.")
                if conn: conn.rollback()
                return jsonify({"error": "Failed to create connection request, no data returned."}), 500
            conn.commit()
            return jsonify({"message": "Connection request sent successfully.", "request": row_to_dict(new_request_raw)}), 201
    except psycopg2.IntegrityError as e: 
        if conn: conn.rollback()
        app.logger.warning(f"[{func_name}] Integrity error (e.g., duplicate request): {e}", exc_info=True)
...
def get_local_org_requests(user_id): 
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            """, (user_id,))
            return jsonify({"pendingRequests": [row_to_dict(req_raw) for req_raw in cur.fetchall()]}), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for local org {user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch pending requests"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for local org {user_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_local_org_connections(user_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            return jsonify([row_to_dict(row_raw) for row_raw in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for local_org_id {user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch connections"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for local_org_id {user_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def respond_to_connection_request(request_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not updated_request_raw:
                app.logger.error(f"[{func_name}] Failed to update request {request_id}, no data returned.")
                if conn: conn.rollback()
                return jsonify({"error": "Failed to update request status."}), 500
            conn.commit()
            return jsonify({"message": f"Request {new_status} successfully.", "request": row_to_dict(updated_request_raw)}), 200
    except psycopg2.Error as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error for request {request_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to respond to request"}), 500
    except Exception as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error for request {request_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_organizer_camp_connections(camp_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            return jsonify([row_to_dict(conn_req) for conn_req in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for organizer {organizer_id}, camp {camp_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch connection statuses"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for organizer {organizer_id}, camp {camp_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_chat_messages(connection_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            """, (connection_id,))
            return jsonify([row_to_dict(msg_raw) for msg_raw in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for connection {connection_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch messages"}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for connection {connection_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def send_chat_message(connection_id):
    ...
    if not sender_id_str: return jsonify({"error": "Unauthorized: Sender ID missing."}), 401
    try: sender_id = int(sender_id_str)
    except ValueError: return jsonify({"error": "Invalid Sender ID format."}), 400
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not new_message_raw:
                app.logger.error(f"[{func_name}] Failed to send message for connection {connection_id}, no data returned.")
                if conn: conn.rollback()
                return jsonify({"error": "Failed to send message, no data returned."}), 500
            
            conn.commit()
            new_message = row_to_dict(new_message_raw)
            cur.execute("SELECT username FROM users WHERE id = %s", (new_message['sender_id'],)) # Get sender name
...
def patient_chatbot():
    ...
    try:
        conn = get_db_connection()
        if not conn: 
            app.logger.error(f"[{func_name}] Database connection failed for chatbot context.")
        else: 
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                if patient_context:
                    patient_name = patient_context['name'] or patient_name
                ...
                cur.execute("""INSERT INTO patient_chat_messages (patient_user_id, patient_record_id, message_text, sender_type, language) 
                               VALUES (%s, %s, %s, 'user', %s)""", 
                            (current_user_id, patient_record_id, user_message, target_language_simple))
                conn.commit()
    except psycopg2.Error as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error fetching patient context or saving user message: {e}", exc_info=True)
    except Exception as e: 
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error during DB interaction for chatbot: {e}", exc_info=True)
    finally:
        if conn and not conn.closed:
            try: conn.close()
            except Exception as e_close: app.logger.error(f"[{func_name}] Error closing DB connection: {e_close}", exc_info=True)
        ...
        try:
            ...
            if conn_store_bot:
                with conn_store_bot.cursor() as cur_store_bot:
                    cur_store_bot.execute("""INSERT INTO patient_chat_messages (patient_user_id, patient_record_id, message_text, sender_type, language) 
                                             VALUES (%s, %s, %s, 'bot', %s)""", 
                                          (current_user_id, patient_record_id, final_bot_response, target_language_simple))
                    conn_store_bot.commit()
            else:
                app.logger.error(f"[{func_name}] Database connection failed for storing bot message.")
        except psycopg2.Error as e_store:
            if conn_store_bot: conn_store_bot.rollback()
            app.logger.error(f"[{func_name}] Database error storing bot chat message: {e_store}", exc_info=True)
        except Exception as e_gen_store:
            if conn_store_bot: conn_store_bot.rollback()
            app.logger.error(f"[{func_name}] Unexpected error storing bot chat message: {e_gen_store}", exc_info=True)
        finally:
            if conn_store_bot and not conn_store_bot.closed:
                try: conn_store_bot.close()
                except Exception as e_close: app.logger.error(f"[{func_name}] Error closing conn_store_bot: {e_close}", exc_info=True)
        
        return jsonify({"reply": final_bot_response, "language": target_language_simple}), 200
    except Exception as e_bot_logic: 
        app.logger.error(f"[{func_name}] Error in chatbot processing logic: {e_bot_logic}", exc_info=True)
        return jsonify({"reply": "Sorry, I encountered an issue processing your request.", "language": target_language_simple}), 500
...
def patient_feedback():
    ...
    if not request.is_json: return jsonify({"error": "Missing JSON in request"}), 400
    data = request.get_json()
    feedback_text, rating, patient_record_id, language = \
        data.get('feedback_text'), data.get('rating'), data.get('patient_record_id'), data.get('language', 'en')
    ...
    try:
        ...
        if not conn: 
            ...
            return jsonify({"error": "Database connection failed."}), 500
        with conn.cursor() as cur:
            cur.execute("INSERT INTO patient_feedback (patient_user_id, patient_record_id, feedback_text, rating, language) VALUES (%s, %s, %s, %s, %s)",
...
def get_all_camps_for_review():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            return jsonify([row_to_dict(camp) for camp in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch camps"}), 500
    except Exception as e:
...
def submit_camp_review():
    ...
    if not request.is_json: return jsonify({"error": "Missing JSON in request"}), 400
    data = request.get_json()
    camp_id, rating, comment = data.get('campId'), data.get('rating'), data.get('comment')
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not user or user['user_type'] != 'requester': 
                return jsonify({"error": "Forbidden: Only registered patients can submit reviews."}), 403
        ...
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error for camp {camp_id}, user {patient_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to submit review."}), 500
    except Exception as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Unexpected error for camp {camp_id}, user {patient_user_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def get_camp_reviews_for_organizer(camp_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            """, (camp_id,))
            return jsonify([row_to_dict(review) for review in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for camp {camp_id}, organizer {organizer_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to fetch reviews."}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for camp {camp_id}, organizer {organizer_user_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
def add_patient_for_followup(camp_id):
    ...
    if not request.is_json: return jsonify({"error": "Missing JSON in request"}), 400
    data = request.get_json()
    patient_identifier, notes = data.get('patientIdentifier'), data.get('notes')
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute("INSERT INTO camp_follow_ups (camp_id, patient_identifier, notes, added_by_organizer_id, linked_patient_user_id) VALUES (%s, %s, %s, %s, %s) RETURNING id, patient_identifier, notes, created_at;",
                        (camp_id, patient_identifier, notes, organizer_user_id, linked_patient_user_id))
            new_followup_raw = cur.fetchone()
            if not new_followup_raw:
                app.logger.error(f"[{func_name}] Follow-up add failed, no data returned.")
                if conn: conn.rollback()
                return jsonify({"error": "Failed to add follow-up, no data returned."}), 500
            conn.commit()
            return jsonify({"message": "Patient added for follow-up.", "follow_up": row_to_dict(new_followup_raw)}), 201
    except psycopg2.Error as e:
        if conn: conn.rollback()
        app.logger.error(f"[{func_name}] Database error for camp {camp_id}, organizer {organizer_user_id}: {e}", exc_info=True)
...
def get_camp_followup_patients(camp_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            return jsonify([row_to_dict(fu) for fu in cur.fetchall()]), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for camp {camp_id}, organizer {organizer_user_id}: {e}", exc_info=True)
...
def check_patient_followup_eligibility():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            else: return jsonify({"eligible": False, "message": "You are not currently scheduled for any follow-ups."}), 200
    except psycopg2.Error as e:
        app.logger.error(f"[{func_name}] Database error for user {patient_user_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to check follow-up eligibility."}), 500
    except Exception as e:
        app.logger.error(f"[{func_name}] Unexpected error for user {patient_user_id}: {e}", exc_info=True)
        return jsonify({"error": "An unexpected server error occurred."}), 500
    finally:
        if conn and not conn.closed:
            try: conn.close()
...
with app.app_context():
    ...
    if not create_tables(): 
        ...
        app.logger.critical("Please check logs for specific database errors (connection, permissions, SQL syntax).")
        app.logger.critical("Ensure DB is running, accessible, and user has permissions.")
        app.logger.critical("############################################################")
    else:
...
if __name__ == '__main__':
    ...
    app.logger.info(f"Indicator JSONs will be sourced from: {BASE_JSON_DIR}")
    app.logger.info(f"Geographic points CSV will be sourced from: {CSV_POINTS_PATH}")
